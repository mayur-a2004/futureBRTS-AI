...
- If the student's answer is correct, give score >= 60, set passed to true.`
    ...
    generateChapterPdf: async (req: Request | any, res: Response) => {
        try {
            ...
            // Title Banner
            doc.rect(40, 40, 515, 60).fillColor('#1e1b4b').fill();
            doc.font('Helvetica-Bold').fontSize(18).fillColor('#ffffff').text(`OFFICIAL TEXTBOOK CHAPTER PDF`, 55, 55);
            doc.font('Helvetica').fontSize(10).fillColor('#fbbf24').text(`BOARD: ${b.toUpperCase()} | CLASS: ${std} | SUBJECT: ${sub.toUpperCase()} | MEDIUM: ${med.toUpperCase()}`, 55, 78);

            doc.moveDown(3);
            doc.font('Helvetica-Bold').fontSize(16).fillColor('#0f172a').text(`Chapter: ${title}`);
            doc.font('Helvetica').fontSize(10).fillColor('#64748b').text(`Curriculum Standard: ${b} Class ${std} (${med} Medium)`);
            doc.moveDown(1);

            doc.rect(40, doc.y, 515, 2).fillColor('#e2e8f0').fill();
            doc.moveDown(1);

            doc.font('Helvetica-Bold').fontSize(13).fillColor('#4f46e5').text('Chapter Subtopics & Micro-Concepts:');
            doc.moveDown(0.5);

            subs.forEach((st: string, idx: number) => {
                doc.font('Helvetica-Bold').fontSize(11).fillColor('#1e293b').text(`${idx + 1}. ${st.trim()}`);
                doc.font('Helvetica').fontSize(10).fillColor('#334155').text(`Explanation: Fundamental theorems, formulas, and step-by-step resolution for ${st.trim()}.`);
                doc.moveDown(0.5);
            });

            doc.moveDown(1);
            doc.font('Helvetica-Bold').fontSize(13).fillColor('#4f46e5').text('Board Exam Weightage & Guidance:');
            doc.moveDown(0.5);
            doc.font('Helvetica').fontSize(10).fillColor('#334155').text(`- Expected Exam Weightage: 8 Marks (Direct Numerical & Proof Questions).`);
            doc.text(`- Medium of Instruction: ${med}. Solved textbook exercises and key definitions included.`);

            doc.moveDown(2);
            doc.font('Helvetica-Oblique').fontSize(9).fillColor('#94a3b8').text(`Generated by Future Education OS • Official ${b} Class ${std} AI Curriculum System`, { align: 'center' });
        ...
        try {
            ...
            return res.json({
                ...
                details: aiOutput || {
                    ...
                    weightage: '8 Marks in Board Examination',
                    keyFormulas: [
