...
export const MessageBubble = React.memo(({
    ...
    let isCurrentlyThinking = isThinking;
    let cleanContentForRender = sanitizedCleanedContent;

    const thinkStart = cleanContentForRender.indexOf('<think>');
    const thinkEnd = cleanContentForRender.indexOf('</think>');

    if (thinkStart !== -1) {
        if (thinkEnd !== -1) {
            displayThinking = cleanContentForRender.slice(thinkStart + 7, thinkEnd);
            cleanContentForRender = cleanContentForRender.slice(0, thinkStart) + cleanContentForRender.slice(thinkEnd + 8);
            isCurrentlyThinking = false;
        } else {
            displayThinking = cleanContentForRender.slice(thinkStart + 7);
            cleanContentForRender = cleanContentForRender.slice(0, thinkStart);
    ...
    React.useEffect(() => {
        ...
        } else if (displayThinking && !isCurrentlyThinking) {
            // Once thinking is done, auto collapse
            setIsThinkCollapsed(true);
        }
    }, [isCurrentlyThinking, !!displayThinking]);
                            ...
                            {displayThinking && (
                                <div className="mb-6 rounded-2xl border border-white/5 bg-[#18181b]/35 backdrop-blur-md overflow-hidden shadow-lg transition-all duration-300">
                                    <div 
                                        ...
                                        className="flex items-center justify-between px-5 py-3.5 bg-white/[0.02] hover:bg-white/[0.04] transition-colors cursor-pointer select-none border-b border-white/[0.02]"
                                    >
                                        <div className="flex items-center gap-3">
                                            <div className="relative flex items-center justify-center">
                                                {isCurrentlyThinking ? (
                                                    <>
                                                        <Brain size={14} className="text-indigo-400 animate-pulse animate-duration-1000" />
                                                        <span className="absolute w-2 h-2 bg-indigo-500 rounded-full animate-ping opacity-75" />
                                                    </>
                                                ) : (
                                            ...
                                            <span className="text-[10px] font-black uppercase tracking-[0.2em] text-gray-400">
                                                {isCurrentlyThinking ? "Thinking Process..." : (thinkingTime ? `Thought for ${thinkingTime}s` : "Thought process completed")}
                                            </span>
                                        </div>
                                        <div className="text-[10px] font-bold text-indigo-400/60 uppercase tracking-widest hover:text-indigo-300 transition-colors flex items-center gap-1">
                                            {isThinkCollapsed ? "Expand" : "Collapse"}
                                        </div>
                                    </div>
                                ...
                                components={{
                                                    ...
                                                    if (typeof child === 'string') {
                                                        ...
                                                        return parts.map((part, i) => {
                                                            if (part.startsWith('[[Citation:')) {
                                                                ...
                                                                return (
                                                                    <CitationTooltip key={i} citation={{ name: finalName, url: sanitizedUrl, snippet: finalSnippet }}>
                                                                        {finalName}
                                                                    </CitationTooltip>
                                                                );
                                                            }
                                    ...
                                    a: ({ href, children }) => {
                                        ...
                                        const isDownload = sanitizedHref?.includes('/download') ||
                                            ...
                                            sanitizedHref?.endsWith('.zip') ||
                                            sanitizedHref?.endsWith('.docx') ||
                                            sanitizedHref?.endsWith('.doc') ||
                                            sanitizedHref?.endsWith('.pptx') ||
                                            sanitizedHref?.endsWith('.ppt');
                                        if (isDownload) {
                                            return (
                                                <a
                                                    href={sanitizedHref}
