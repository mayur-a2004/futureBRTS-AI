...
import { School, ShieldCheck, GraduationCap, Lock, LogOut } from 'lucide-react';
...
export const SchoolContextBar: React.FC<SchoolContextBarProps> = ({
  ...
  const handleTeacherLogout = () => {
    ...
    window.location.href = '/future-education/teacher';
  };
  ...
  return (
    <div className="p-5 rounded-3xl bg-gradient-to-r from-zinc-950 via-purple-950/40 to-zinc-950 border border-purple-500/20 shadow-xl mb-6 flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
      ...
      {/* Role Badge, Class Selector & Logout */}
        ...
        {userRole === 'TEACHER' ? (
          <div className="flex items-center gap-2">
            ...
            <button
              ...
              className="px-3.5 py-2 rounded-2xl bg-rose-500/10 hover:bg-rose-500/20 text-rose-300 border border-rose-500/30 text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer"
              title="Log out teacher account"
            >
          ...
          <div className="flex items-center gap-2">
            ...
            {/* 👨‍🏫 Faculty Portal Login Button */}
            <button
              onClick={() => window.location.href = '/future-education/teacher'}
              className="px-3.5 py-2 rounded-2xl bg-purple-500/10 hover:bg-purple-500/20 text-purple-300 border border-purple-500/30 text-xs font-bold flex items-center gap-1.5 transition-all cursor-pointer"
              title="Faculty / Teacher Login & Register Portal"
            >
              <GraduationCap size={14} className="text-purple-400" />
